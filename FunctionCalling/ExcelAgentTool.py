...
def excel_agent_tool(content: str) -> str:
    ...
    调用示例：
    - "将 ./test_data.xlsx 导入到 daijia_coupon.coupon_info 表，列映射为：优惠券类型→coupon_type, 优惠券名字→name, 金额→amount, 折扣→discount, 使用门槛→condition_amount, 发行数量→publish_count, 每人限领→per_limit, 已使用数量→use_count, 领取数量→receive_count, 过期时间→expire_time, 优惠券描述→description, 状态→status, 删除标记→is_deleted"
